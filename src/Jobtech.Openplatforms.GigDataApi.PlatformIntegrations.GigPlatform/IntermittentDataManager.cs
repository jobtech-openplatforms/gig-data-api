...
namespace Jobtech.OpenPlatforms.GigDataApi.PlatformIntegrations.GigPlatform
{
    public class IntermittentDataManager
    {
        ...
        public async Task RegisterRequestData(string requestId, string userId, string platformId)
        {
            var dataObj = new RequestData {UserId = userId, PlatformId = platformId};
            await _cache.SetStringAsync(requestId, JsonConvert.SerializeObject(dataObj));
        }

        public async Task<(string UserId, string PlatformId)> GetRequestData(string requestId)
        {
            ...
            return (requestData.UserId, requestData.PlatformId);
        }
    }
