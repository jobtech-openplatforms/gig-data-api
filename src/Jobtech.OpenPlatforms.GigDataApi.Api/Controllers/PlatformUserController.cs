namespace Jobtech.OpenPlatforms.GigDataApi.Api.Controllers
{
    ...
    public class PlatformUserController : ControllerBase
    {
        ...
        public PlatformUserController(IDocumentStore documentStore, IHttpContextAccessor httpContextAccessor,
            ...
            IPlatformManager platformManager, IPlatformDataManager platformDataManager, IUserManager userManager, IAppManager appManager)
        {
        ...
        public async Task<StartPlatformOauthConnectionResultViewModel> StartConnectUserToOauthPlatform(StartPlatformConnectionOauthModel model)
        {
            var uniqueUserIdentifier = _httpContextAccessor.HttpContext.User.Identity.Name;

            using (var session = _documentStore.OpenAsyncSession())
            {
                var existingUser = await _userManager.GetUserByUniqueIdentifier(uniqueUserIdentifier, session);

                var app = await _appManager.GetAppFromApplicationId(model.ApplicationId, session);

                var authorizationResult = await _platformConnectionManager.StartConnectUserToOauthPlatform(model.PlatformId, existingUser, app,
                    model.CallbackUri, session);

                await session.SaveChangesAsync();

                return new StartPlatformOauthConnectionResultViewModel(authorizationResult.State, authorizationResult.OAuthAuthenticationUrl);
            }
        }
        ...
        public async Task<ActionResult> RequestPlatformDataUpdateNotification(PlatformDataUpdateRequestModel model)
        {
            ...
            using (var session = _documentStore.OpenAsyncSession())
            {
                ...
                var platform = await _platformManager.GetPlatformByExternalId(model.PlatformId, session);

                var platformConnectionInfo = _platformConnectionManager.GetPlatformConnectionInfo(existingUser, platform.Id);
                ...
                var platformData = await _platformDataManager.GetPlatformData(existingUser.Id, platform.Id, session);
        ...
        public async Task<PlatformConnectionResultViewModel> ConnectUserToEmailPlatform(
            StartPlatformConnectionEmailModel model)
        {
            var uniqueUserIdentifier = _httpContextAccessor.HttpContext.User.Identity.Name;
            ...
            using (var session = _documentStore.OpenAsyncSession())
            {
                ...
                var result = await _platformConnectionManager.ConnectUserToEmailPlatform(model.PlatformId, existingUser, app,
                    model.PlatformUserEmailAddress, session);
